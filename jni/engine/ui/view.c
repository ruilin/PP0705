...
PUBLIC BOOL view_onEvent(View *view, View *superView, MOTION_ACTION action, int x, int y) {
	if (FALSE == view->isShowing) return FALSE;
	void *iterator = linked_list_iterator(view->linked_subView, LINKED_LIST_HEAD);
	while (NULL != iterator) {
		View *subView = linked_list_iterator_next(&iterator);
...
PUBLIC void view_addSubView(View *view, View *subView) {
	...
	if (NULL != subView->callback_onLayout) {
		subView->callback_onLayout(subView->instance, (NULL != view ? view->instance : NULL), subView->rect);
	} else {
...
PRIVATE void view_rectInView(View *view, Rect *originalRect, Rect *newRect) {
	rect_set(newRect, view->rect.left + originalRect->left, view->rect.top + originalRect->top, originalRect->width, originalRect->height);
	return;
}
